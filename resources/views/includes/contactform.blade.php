<div class="isolate bg-gray-900 px-6 py-24 sm:py-32 lg:px-8">
    ...
    <form action="{{ route('contacts.post') }}" method="POST" class="mx-auto mt-16 max-w-xl sm:mt-20">
        ...
        <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
            ...
            <div>
                <label for="name" class="block text-sm/6 font-semibold text-white">{{ __('general.Name') }}</label>
                <div class="mt-2.5">
                    <input id="name" type="text" name="name" placeholder="{{ __('general.Input name') }}" value="{{ old('name') }}" autocomplete="given-name" class="block w-full rounded-md bg-white/5 px-3.5 py-2 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500" />
                </div>
            </div>
            <div>
                <label for="email" class="block text-sm/6 font-semibold text-white">{{ __('general.e-mail') }}</label>
                <div class="mt-2.5">
                    <input id="email" type="email" name="email" placeholder="{{ __('general.Input e-mail') }}" value="{{ old('email') }}" autocomplete="email" class="block w-full rounded-md bg-white/5 px-3.5 py-2 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500" />
                </div>
            </div>
            <div>
                <label for="subject" class="block text-sm/6 font-semibold text-white">{{ __('general.Subject message') }}</label>
                <div class="mt-2.5">
                    <input id="subject" type="text" name="subject" placeholder="{{ __('general.Input subject message') }}" value="{{ old('subject') }}" class="block w-full rounded-md bg-white/5 px-3.5 py-2 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500" />
                </div>
            </div>

            <div class="sm:col-span-2">
                <label for="message" class="block text-sm/6 font-semibold text-white">{{ __('general.Message') }}</label>
                <div class="mt-2.5">
                    <textarea id="message" name="message" rows="4" placeholder="{{ __('general.Input message') }}" class="block w-full rounded-md bg-white/5 px-3.5 py-2 text-base text-white outline-1 -outline-offset-1 outline-white/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500">{{ old('message') }}</textarea>
                </div>
            </div>
